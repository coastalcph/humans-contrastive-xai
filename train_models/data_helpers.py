...
def filter_out_sst2(original_dataset):
    ...
    excl_dataset = load_dataset("coastalcph/fair-rationales", "sst2")
    doc_set = [
        re.sub(r"\s", "", sentence.lower())
        for sentence in excl_dataset["train"]["sentence"]
    ]
    ...
    with open(
        os.path.join(
            DATA_DIR, "rationales-multilingual", "Multilingual_Interpretability_EN.csv"
        ),
        "r",
        encoding="utf-8",
        errors="ignore",
    ...
    doc_set += documents
    doc_set = set(doc_set)
    modified_dataset = original_dataset.filter(
        lambda example: re.sub(r"\s", "", example["sentence"]) not in doc_set,
...
def filter_out_dynasent(original_dataset):
    # open Laura's rationales-demographics annotated dataset
    excl_dataset = load_dataset("coastalcph/fair-rationales", "dynasent")
    doc_set = [
        re.sub(r"\s", "", sentence.lower())
        for sentence in excl_dataset["train"]["sentence"]
    ]
...
def fix_dynasent(original_dataset):
    ...
    modified_dataset = original_dataset.filter(
        lambda example: example["gold_label"] != "neutral", load_from_cache_file=False
    )
    ...
    modified_dataset = modified_dataset.remove_columns(
        [
            column_name
            for column_name in modified_dataset.column_names
            if column_name not in ["sentence", "label"]
        ]
    )
...
def get_char_scores(words, word_scores, sentence, tokenizer):
    ...
    char_scores = " ".join(
        ["".join([str(score)] * len(word)) for word, score in zip(words, word_scores)]
    )
    inputs = tokenizer(sentence, return_offsets_mapping=True)
    inputs["input_ids_scores"] = []
    _ = [
        inputs["input_ids_scores"].append(char_scores[off[0] : off[1]])
        for off in inputs["offset_mapping"]
    ]
    # Proc character-level scores back to list of integers
    try:
        inputs["input_ids_scores"] = [
            0 if len(x) == 0 else int(x[0]) for x in inputs["input_ids_scores"]
        ]
    except:
        import pdb
...
def load_sst2_rationales(model_path: str, rationales_preview=False):
    ...
    def align_words_rationales(example):
        ...
        rationale_indices = [
            int(index)
            for index in example["rationale_index"]
            .replace("[", "")
            .replace("]", "")
            .split(",")
        ]
        example["word_scores"] = [
            1 if token_idx in rationale_indices else 0
        ...
        example["input_ids"] = [
            ...
            for sub_word in tokenizer.tokenize(word)
        ]
        # except AttributeError:
        #    import pdb;pdb.set_trace()
        example["rationale_ids"] = [
            token_score
            for token_score, word in zip(example["word_scores"], example["words"])
            for _ in tokenizer.tokenize(word)
        ]
        example["tokenized_inputs"] = get_char_scores(
            words, example["word_scores"], example["sentence"], tokenizer
        )
        return example

    # open Laura's rationales-demographics annotated dataset
    dataset = load_dataset(
        os.path.join(DATA_DIR, "rationales-demographics"),
        data_files={"train": "SST_annot_after_processing.csv"},
        cache_dir=None,
    )["train"]

    dataset = dataset.filter(
        lambda example: example["label"] != "no sentiment", load_from_cache_file=False
    )
    ...
    dataset = dataset.remove_columns(
        [
            ...
            if column_name not in ["sentence", "label", "rationale_index", "rationale"]
        ]
    )
    ...
    if rationales_preview:
        for sample in dataset:
            rationale = " ".join(
                [
                    f"[{token}]" if token_score else token
                    for token_score, token in zip(
                        sample["word_scores"], sample["words"]
                    )
                ]
            )
...
def load_dynasent_rationales(model_path: str, rationales_preview=False):
    ...
    def align_words_rationales(example):
        ...
        sentence = " ".join(
            words
        )  # harmonize sentences in comparison to example['sentence']
        example["label"] = 1 if example["label"] == "positive" else 0
        example["words"] = words
        rationale_indices = [
            int(index)
        ...
        example["tokenized_inputs"] = get_char_scores(
            words, example["word_scores"], sentence, tokenizer
        )
    ...
    dataset = load_dataset(
        os.path.join(DATA_DIR, "rationales-demographics"),
        data_files={"train": "dynasent_annot_after_processing.csv"},
        cache_dir=None,
    )["train"]
    dataset = dataset.filter(
        lambda example: example["label"] != "no sentiment", load_from_cache_file=False
...
def load_biosbias_rationales(model_path: str, rationales_preview=False):
    ...
    def extract_rationales(example):
        if example["spans"] is not None:
            ...
            assert sum(tokens_scores[:question_end]) == 0.0
    ...
    dataset = load_dataset(
        ...
        data_files=os.path.join(
            DATA_DIR, "rationales-biosbias/bios_filtered_annotations.jsonl"
        ),
        cache_dir=None,
    )["train"]
    dataset = dataset.filter(
        lambda example: example["spans"] is not None, load_from_cache_file=False
    )
    ...
    dataset = dataset.filter(
        lambda example: len(example["words"]) > 0, load_from_cache_file=False
    )
    ...
    if True:
        easy_dataset = load_dataset(
            "json",
            data_files=os.path.join(
                DATA_DIR, "rationales-biosbias/bios_easy_annotations.jsonl"
            ),
            cache_dir=None,
        )["train"]
        easy_dataset = easy_dataset.map(extract_rationales, load_from_cache_file=False)
        easy_dataset = easy_dataset.remove_columns(
            [
                column_name
                for column_name in easy_dataset.column_names
                if column_name
                not in [
                    "words",
                    "word_scores",
        ...
        easy_dataset = easy_dataset.filter(
            lambda example: len(example["words"]) > 0, load_from_cache_file=False
        )
