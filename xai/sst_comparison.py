...
def rat_num(len_sen, rationale):
    try:
        rationale = (
            [el - 1 for el in list(map(int, rationale.split(",")))]
            if len(rationale.split(",")) > 1
            else [int(float(rationale)) - 1]
        )
...
@click.option(
    "--modelname",
    default="coastalcph/roberta-base-sst2",
    help="path to finetuned model on hf",
)
@click.option("--model_type", default="roberta", help="Model type")
@click.option(
    "--importance_aggregator", default="max", help="aggregation method (max, mean, sum)"
)
...
def main(modelname, model_type, importance_aggregator, results_dir_base, xai_method):
    ...
    for dataset_name in ["sst2", "dynasent"]:
        ...
        data = load_dataset("coastalcph/fair-rationales", dataset_name)
        subset = data["train"].filter(
            lambda ex: ex["attentioncheck"] == "PASSED"
            and ex["label"] == ex["original_label"]
        )
        annotation = pd.DataFrame(subset)
        annotation["len_sen"] = annotation["sentence"].apply(
            lambda x: len(x.split(" "))
        )
        annotation["rationale_numeric"] = annotation.apply(
            lambda x: rat_num(x.len_sen, x.rationale_index), axis=1
        )
        annotation["normalized_text"] = annotation["sentence"].apply(
            lambda x: re.sub("[^a-z]", "", "".join(x).lower())
        )
        ...
        relevance_dict["non-contrastive"] = pickle.load(
            open(
                join(results_dir, f"relevance_true_{xai_method}_{modelbase}.pkl"), "rb"
            )
        ).drop_duplicates(subset=["tokens"])
        relevance_dict["contrastive"] = pickle.load(
            open(
                join(
                    results_dir, f"relevance_contrastive_{xai_method}_{modelbase}.pkl"
                ),
                "rb",
            )
        ).drop_duplicates(subset=["tokens"])
        relevance_dict_val["contrastive"] = pickle.load(
            open(
                join(
                    results_dir,
                    f"relevance_validation_contrastive_{xai_method}_{modelbase}.pkl",
                ),
                "rb",
            )
        ).drop_duplicates(subset=["tokens"])
        relevance_dict_val["non-contrastive"] = pickle.load(
            open(
                join(
                    results_dir,
                    f"relevance_validation_true_{xai_method}_{modelbase}.pkl",
                ),
                "rb",
            )
        ).drop_duplicates(subset=["tokens"])
        relevance_dict["contrastive"] = pd.concat(
            [relevance_dict["contrastive"], relevance_dict_val["contrastive"]]
        )
        ...
        df_results[dataset_name] = pd.DataFrame(
            columns=["gaze", "annotations", "non-contrastive", "contrastive"]
        )
        y_pred_old = []
        y_pred_cont_old = []
        for mode in relevance_dict:
            ...
            y_pred_cont_gaze = []
            for ix, xx in tqdm(
                relevance_dict[mode].iterrows(), desc="iterating through datapoints"
            ):
                ...
                if len(annotation.query("normalized_text==@normalized_text")) > 0:
                    ...
                    rat_tmp = np.zeros(
                        [len(ann_tmp), len(ann_tmp.iloc[0]["rationale_numeric"])]
                    )
                    ...
                    words, word_importances = merge_subwords_words(
                        xx["tokens"],
                        ann_tmp.iloc[0]["sentence"].split(" "),
                        xx["attention"],
                    ...
                    if len(words) == len(gold_rationale):
                        idx_sorted = np.argsort(word_importances)[::-1][
                            : sum(gold_rationale)
                        ]
                    ...
                    if dataset_name == "sst2":
                        try:
                            ...
                            gaze_tmp = df_gaze.query("text_id==@text_id").iloc[0][
                                "relfix"
                            ]
                            if len(gold_rationale) == len(gaze_tmp):
                                idx_sorted = np.argsort(gaze_tmp)[::-1][
                                    : sum(gold_rationale)
                                ]
            ...
            df_results[dataset_name].loc[mode, "annotations"] = np.around(
                cohen_kappa_score(y_true, y_pred), decimals=2
            )
            df_results[dataset_name].loc["annotations", mode] = np.around(
                cohen_kappa_score(y_true, y_pred), decimals=2
            )
            ...
            if dataset_name == "sst2":
                ...
                df_results[dataset_name].loc["gaze", mode] = np.around(
                    cohen_kappa_score(y_gaze, y_pred_gaze), decimals=2
                )
                df_results[dataset_name].loc[mode, "gaze"] = np.around(
                    cohen_kappa_score(y_gaze, y_pred_gaze), decimals=2
                )
                ...
                df_results[dataset_name].loc["annotations", "gaze"] = np.around(
                    cohen_kappa_score(y_true_gaze, y_gaze), decimals=2
                )
                df_results[dataset_name].loc["gaze", "annotations"] = np.around(
                    cohen_kappa_score(y_true_gaze, y_gaze), decimals=2
                )

                print(
                    "gaze cont vs model cont", spearmanr(y_gaze_cont, y_pred_cont_gaze)
                )

            y_pred_old = y_pred if len(y_pred_old) == 0 else y_pred_old
            y_pred_cont_old = (
                y_pred_cont if len(y_pred_cont_old) == 0 else y_pred_cont_old
            )
        ...
        df_results[dataset_name].loc["contrastive", "non-contrastive"] = np.around(
            cohen_kappa_score(y_pred, y_pred_old), decimals=2
        )
    ...
    df_plot = (
        ...
        .fillna(0)
        .loc[
            ["gaze", "annotations", "contrastive"],
            ["annotations", "contrastive", "non-contrastive"],
        ]
    )
    ...
    df_plot = (
        df_results["dynasent"]
        .fillna(0)
        .loc[["annotations", "contrastive"], ["contrastive", "non-contrastive"]]
    )
    mask = np.triu(np.ones_like(df_results["dynasent"], dtype=bool))[:-1, :-2]
    sns.heatmap(
        df_plot, annot=True, fmt=".2f", square=True, vmin=vmin, cmap="mako", mask=~mask
    )
    ...
    cbar_ax = fig.add_axes([0.8, 0.2, 0.03, 0.6])
    ...
    df_plot = (
        ...
        .loc[
            ["annotations", "contrastive", "non-contrastive"],
            ["contrastive", "non-contrastive", "gaze"],
        ]
    )
    mask = np.triu(np.ones_like(df_results["sst2"], dtype=bool))
    sns.heatmap(
    ...
    axs[0].set_xticks(
        [0.5, 1.5, 2.5], ["contr.lrp", "non-contr.lrp", "gaze"], rotation=30, ha="right"
    )
    ...
    axs[1].set_xticks(
        [0.5, 1.5, 2.5], ["contr.lrp", "non-contr.lrp", ""], rotation=30, ha="right"
    )
